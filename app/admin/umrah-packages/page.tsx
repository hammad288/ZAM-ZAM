...
export default async function AdminUmrahPackagesPage() {
  let packages: Awaited<ReturnType<typeof getAllUmrahPackages>> = []
  try { packages = await getAllUmrahPackages() } catch {}

  return (
    <div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-emerald-900" style={{ fontFamily: 'Cinzel, serif' }}>Umrah Packages</h1>
          <p className="text-gray-500 mt-1">{packages.length} total packages</p>
        </div>
        <Link href="/admin/umrah-packages/new" className="btn-primary text-sm px-5 py-2.5"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        {packages.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-4">No Umrah packages yet</p>
            <Link href="/admin/umrah-packages/new" className="btn-primary text-sm"
              style={{ background: 'linear-gradient(135deg, #022c22, #065f46)' }}>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-4 font-semibold text-gray-700">Tags</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {packages.map((pkg) => (
                  <tr key={pkg.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4"><span className="badge-green text-xs">{pkg.umrahType.replace('_', ' ')}</span></td>
                    <td className="px-4 py-4 font-semibold text-emerald-800">{formatCurrency(Number(pkg.price))}</td>
                    <td className="px-4 py-4 text-gray-600">{pkg.duration} days</td>
                    <td className="px-4 py-4">
                      <div className="flex gap-1 flex-wrap">
                        {pkg.isRamzan && <span className="text-xs bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded-full">🌙 Ramzan</span>}
                        {pkg.isFamilyPackage && <span className="text-xs bg-pink-100 text-pink-700 px-1.5 py-0.5 rounded-full">👨‍👩‍👧 Family</span>}
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        {pkg.featured && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">⭐</span>}
                      </div>
                    </td>
                    <td className="px-4 py-4">
